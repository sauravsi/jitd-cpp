...
unsigned long int treeBuilder::crackDepthFirst(){
	using clock = std::chrono::steady_clock;
	cog** node = dfc.front();
	dfc.pop();
	...
	if(left->size() != 0 && right->size() != 0){
		...
		*node = new btreeNode<int>(key, l, r);
		clock::time_point end = clock::now();
        clock::duration execution_time = end - start;
		dfc.push(&(((btreeNode<int>*)*node)->left));
		dfc.push(&(((btreeNode<int>*)*node)->right));
		return chrono::duration_cast<chrono::nanoseconds>(execution_time).count();
	}
	else {
		uncracked.push(node);
...
unsigned long int treeBuilder::sortDepthFirst(){
	using clock = std::chrono::steady_clock;
	cog** node = dfc.front();
	clock::time_point start = clock::now();
	vector<int>* v = ((arrayNode<int>*)*node)->data;
